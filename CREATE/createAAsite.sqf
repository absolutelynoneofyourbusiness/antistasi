...
if (_hasSPAA) then {
	waitUntil {sleep 3;
		...
		{((count ((_markerPos nearEntities [baseClasses_PLAYER, (_size max 200)]) select {_x getVariable [ "BLUFORSpawn",false ]})) > (3*count (_allSoldiers select {alive _x AND !captive _x})) AND !(alive _SPAA))} OR
